<template>
    <div>
        <section class="hero is-primary is-rounded">
            <div class="hero-body">
                <p class="title">
                    Validador de documentos &nbsp; <el-tooltip class="item" effect="dark"
                                content="Los comprobantes se deben haber generado en entorno producción"
                                placement="top-start">
                    ...
                    <tr slot-scope="{ index, row }">
                        ...
                        <td>
                            <span class="badge bg-secondary text-white"
                                :class="{ 'bg-danger': (row.state_type_id === '11'), 'bg-warning': (row.state_type_id === '13'), 'bg-secondary': (row.state_type_id === '01'), 'bg-info': (row.state_type_id === '03'), 'bg-success': (row.state_type_id === '05'), 'bg-secondary': (row.state_type_id === '07'), 'bg-dark': (row.state_type_id === '09') }">{{ row.state_type_description }}</span>
                        </td>

                        <td>
                            <span class="badge bg-secondary text-white"
                                :class="{
                                    'bg-danger': (row.state_type_sunat_description == 'ANULADO'),
                                    ...
                                    'bg-info': (row.state_type_sunat_description == 'NO EXISTE')
                                }">{{ row.state_type_sunat_description }}</span>
                        </td>
