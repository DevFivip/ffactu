<template>
        <div class="card mb-0 pt-2 pt-md-0">
                                    ...
                                    <table class="table">
                                        ...
                                        <tbody>
                                            <tr v-for="(row, index) in form.items" :key="index">
                                                <td>{{ index + 1 }}</td>
                                                <td>{{ row.item.description }}<br /><small>{{
                                                ...
                                                <td class="text-right">{{ currency_type.symbol }} {{ row.total_discount
                                                }}</td>
                                                <td class="text-right">{{ currency_type.symbol }} {{ row.total_charge }}
                                                </td>
            ...
            <fa-purchase-form-item :showDialog.sync="showDialogAddItem" :currency-type-id-active="form.currency_type_id"
                :exchange-rate-sale="form.exchange_rate_sale" @add="addRow"></fa-purchase-form-item>

            <person-form :showDialog.sync="showDialogNewPerson" type="suppliers" :input_person="input_person"
                :external="true"></person-form>

            <fa-purchase-options :showDialog.sync="showDialogOptions" :recordId="purchaseNewId"
                :type="id ? 'edit' : 'create'" :showClose="false"></fa-purchase-options>
        </div>
</template>
