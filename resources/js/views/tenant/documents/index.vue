    ...
    <div>
        <section class="hero is-primary is-rounded">
            <div class="hero-body">
                ...
                <p class="subtitle">
                    Facturas - Notas - Boletas - Anulaciones
                </p>
                <div class="buttons" v-if="typeUser != 'integrator'">
                    ...
                    <span v-if="import_documents_second == true">
                        <button type="button" class="btn btn-custom btn-sm  mt-2 mr-2"
                            @click.prevent="clickImportSecond()"><i class="fa fa-upload"></i> Importar Formato
                            2</button>
                    </span>
                    <a class="button is-white" :href="`/${resource}/create`">Nuevo</a>

                    <b-dropdown aria-role="list">
                        <template #trigger="{ active }">
                            <b-button label="Reporte de Pagos" type="is-primary" inverted outlined
            ...
            <div class="right-wrapper pull-right" v-if="typeUser != 'integrator'">
                ...
                <div class="btn-group flex-wrap">
                    <button type="button" class="btn btn-custom btn-sm  mt-2 mr-2 dropdown-toggle"
                        data-toggle="dropdown" aria-expanded="false"><i class="fa fa-money-bill-wave-alt"></i> Reporte
                        de Pagos <span class="caret"></span></button>
        ...
        <div class="card mb-0">
            <div class="data-table-visible-columns">
                ...
                <b-dropdown aria-role="list" slot="dropdown" :close-on-click="false">
                    <template #trigger="{ active2 }">
                        <b-button label="Listar Columnas" type="is-primary"
                            :icon-right="active2 ? 'menu-up' : 'menu-down'" />
                    </template>
...
<!-- 
                ...
                <data-table :resource="resource">
                    ...
                    <tr slot-scope="{ index, row }" :class="{
                        ...
                        <td class="text-center">
                            ...
                            <button type="button" style="min-width: 41px"
                                class="btn waves-effect waves-light btn-xs btn-info m-1__2"
                                @click.prevent="clickDownload(row.download_pdf)" v-if="row.has_pdf">PDF</button>
                            <button type="button" style="min-width: 41px"
                                class="btn waves-effect waves-light btn-xs btn-info m-1__2"
                        ...
                        <td class="text-right" v-if="typeUser != 'integrator'">
                            ...
                            <button type="button" class="btn waves-effect waves-light btn-xs btn-info m-1__2"
                                @click.prevent="clickReStore(row.id)" v-if="row.btn_recreate_document">Volver a
                                recrear</button>
                            <button type="button" class="btn waves-effect waves-light btn-xs btn-danger m-1__2"
                                @click.prevent="clickVoided(row.id)" v-if="row.btn_voided">Anular</button>
                            ...
                            <button type="button" class="btn waves-effect waves-light btn-xs btn-info m-1__2"
                                @click.prevent="clickSendOnline(row.id)" v-if="isClient && !row.send_server">Enviar
                                Servidor</button>
                            <button type="button" class="btn waves-effect waves-light btn-xs btn-info m-1__2"
                                @click.prevent="clickCheckOnline(row.id)"
                                v-if="isClient && row.send_server && (row.state_type_id === '01' || row.state_type_id === '03')">Consultar
                                Servidor</button>
                            <button type="button" class="btn waves-effect waves-light btn-xs btn-info m-1__2"
            ...
            <document-constancy-detraction :showDialog.sync="showDialogCDetraction" :recordId="recordId">
